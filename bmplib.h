...
int readRGBBMP(const char filename[], unsigned char*** inputImage);

// write full-color image to the file specified by filename, from outputImage
int writeRGBBMP(const char filename[], unsigned char*** outputImage);

// display full-color image with eog, pause 0.2 seconds. (uses a temp file)
void showRGBBMP(unsigned char*** inputImage);
